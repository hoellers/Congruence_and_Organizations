...
parameters {
  ...
  //including attributes in outcome models
  vector[K] beta_w1;
  array[L] vector[K] beta_wmech;
}
...
model {
  ...
  for (n in 1:N_full){
    w_1[n] ~ normal(gamma[1] + alpha[i_full[n]] + gamma[2] * distance_full[n] + X_full[n] * beta_w1, w_1_sigma);
  }
  ...
  for (l in 1:L){
    w_mech[,l] ~ normal(zeta[l][1] + delta[l][i_mech] + zeta[l][2] * distance_mech + X_mech * beta_wmech[l], w_mech_sigma[l]);
  }
