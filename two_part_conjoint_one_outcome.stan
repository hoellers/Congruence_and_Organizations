...
parameters {
  ...
  vector[2] gamma; //  forced choice logit coefficients
  //vector[2] nu; // meetingOrg outcome coefficients
...
model {
  ...
  //for (n in 1:N_full){
  //  w_1[n] ~ normal(nu[1] + alpha[i_full[n]] + nu[2] * distance_full[n], w_1_sigma);
  //}
  ...
  w_3 ~ bernoulli_logit(tau[i_forced] + gamma[1] + gamma[2] * diff_in_pos);
